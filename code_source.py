...
class Annonce(Base):
    ...
    id_annonces = Column(Integer, primary_key=True)
    title = Column(String)
    price = Column(String)
    ...
    link = Column(String)
    city_id = Column(Integer, ForeignKey('City.id_city'))
...
class Table_associative(Base):
    ...
    annonce_id = Column(Integer, ForeignKey('Annonce.id_annonces'), primary_key=True)
...
for index, row in df.iterrows():
    try:
        ...
        annonce = Annonce(
            ...
            city_id=city.id_city  # Reference the city's ID
        )
        session.add(annonce)
        session.commit()
        ...
        for column in columns_to_extract:
            if row[column] == True:  # Check if the equipment is available
                ...
                if equipment:
                    ...
                    association = Table_associative(annonce_id=annonce.id_annonces, equipment_id=equipment.id)
